<text id="ParlaMint-TR_T27-tbmm-2021-03-18.ana" subcorpus="Reference" term="27" session="61" meeting="-" sitting="-" agenda="-" from="2014-08-25" to="2014-08-25" title="The Turkish parliamentary corpus ParlaMint-TR, Session 61 [ParlaMint.ana SAMPLE]">
<note type="time">
/14.01/	-	-	-	-	-	-	-	-	-	-
</note>
<note type="chair">
/Celal A/	-	-	-	-	-	-	-	-	-	-
</note>
<speech id="tbmm-2021-03-18s01p001" speaker_role="Regular" speaker_id="Celal.Adan" speaker_name="Adan, Celal" speaker_type="MP" speaker_party="MHP" speaker_party_name="Milliyetçi Hareket Partisi" speaker_gender="M" speaker_birth="?">
<p id="tbmm-2021-03-18s01p001.seg0" lang="Turkish">
<s id="tbmm-2021-03-18s01p001-000010">
<name type="ORG">
Türkiye	Türkiye	Türkiye	PROPN	Case=Nom Number=Sing	t1	nmod	birleşim	NOUN	Case=Acc Number=Sing Number[psor]=Sing Person[psor]=2	t6
Büyük	Büyük	büyük	ADJ	-	t2	amod	millet	NOUN	Case=Nom Number=Sing	t3
Millet	Millet	millet	NOUN	Case=Nom Number=Sing	t3	nmod	meclis	NOUN	Case=Gen Number=Sing Number[psor]=Sing Person[psor]=2	t4
Meclisinin	Meclisinin	meclis	NOUN	Case=Gen Number=Sing Number[psor]=Sing Person[psor]=2	t4	nmod	birleşim	NOUN	Case=Acc Number=Sing Number[psor]=Sing Person[psor]=2	t6
</name>
61’inci	61’inci	61	ADJ	Case=Nom NumType=Ord Number=Sing	t5	amod	birleşim	NOUN	Case=Acc Number=Sing Number[psor]=Sing Person[psor]=2	t6
Birleşimini	Birleşimini	birleşim	NOUN	Case=Acc Number=Sing Number[psor]=Sing Person[psor]=2	t6	obj	aç	VERB	Aspect=Prog Evident=Fh Mood=Ind Number=Sing Person=1 Tense=Pres	t7
açıyorum	açıyorum	aç	VERB	Aspect=Prog Evident=Fh Mood=Ind Number=Sing Person=1 Tense=Pres	t7	root	-	-	-	-
<g/>
.	.	.	PUNCT	-	t8	punct	aç	VERB	Aspect=Prog Evident=Fh Mood=Ind Number=Sing Person=1 Tense=Pres	t7
</s>
</p>
</speech>
<speech id="tbmm-2021-03-18s01p002" speaker_role="Regular" speaker_id="Celal.Adan" speaker_name="Adan, Celal" speaker_type="MP" speaker_party="MHP" speaker_party_name="Milliyetçi Hareket Partisi" speaker_gender="M" speaker_birth="?">
<p id="tbmm-2021-03-18s01p002.seg0" lang="Turkish">
<s id="tbmm-2021-03-18s01p002-000010">
Başkan	Başkan	başkan	NOUN	-	t1	nmod:poss	et	VERB	Case=Abl Number=Sing Number[psor]=Sing Person[psor]=3 Polarity=Neg Tense=Past VerbForm=Part	t5
<g/>
lık	lık	lik	ADJ	-	t2	amod	divan	NOUN	Case=Nom Number=Sing Number[psor]=Sing Person[psor]=3	t3
<name type="ORG">
Divanı	Divanı	divan	NOUN	Case=Nom Number=Sing Number[psor]=Sing Person[psor]=3	t3	nmod	teşekkül	NOUN	Case=Nom Number=Sing	t4
</name>
teşekkül	teşekkül	teşekkül	NOUN	Case=Nom Number=Sing	t4	obj	et	VERB	Case=Abl Number=Sing Number[psor]=Sing Person[psor]=3 Polarity=Neg Tense=Past VerbForm=Part	t5
etmediğinden	etmediğinden	et	VERB	Case=Abl Number=Sing Number[psor]=Sing Person[psor]=3 Polarity=Neg Tense=Past VerbForm=Part	t5	nmod	başla	VERB	Aspect=Prog Evident=Fh Mood=Pot Number=Plur Person=1 Polarity=Neg Tense=Pres	t7
çalışmalarımıza	çalışmalarımıza	çalışma	NOUN	Case=Dat Number=Plur Number[psor]=Plur Person[psor]=1	t6	obj	başla	VERB	Aspect=Prog Evident=Fh Mood=Pot Number=Plur Person=1 Polarity=Neg Tense=Pres	t7
başlayamıyoruz	başlayamıyoruz	başla	VERB	Aspect=Prog Evident=Fh Mood=Pot Number=Plur Person=1 Polarity=Neg Tense=Pres	t7	root	-	-	-	-
<g/>
.	.	.	PUNCT	-	t8	punct	başla	VERB	Aspect=Prog Evident=Fh Mood=Pot Number=Plur Person=1 Polarity=Neg Tense=Pres	t7
</s>
</p>
</speech>
<note type="gap::editorial">
/SAMPLING/	-	-	-	-	-	-	-	-	-	-
</note>
</text>
