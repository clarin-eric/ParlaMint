...
<xsl:template match="text()" mode="serialize">
  ...
  <xsl:function name="mk:affiliation-role-test">
    ...
    <xsl:choose>
      ...
      <xsl:when test="$orgrole = 'parliament' and not(contains(' head member deputyHead ', mk:borders($role)))">15:INFO)unexpected affiliation role with parliament organization</xsl:when>
      <xsl:when test="$orgrole = 'government' and not(contains(' head member deputyHead minister deputyMinister ', mk:borders($role)))">16:INFO)unexpected affiliation role with government organization</xsl:when>
      <xsl:when test="$orgrole = 'parliamentaryGroup' and not(contains(' head deputyHead member ', mk:borders($role)))">17:INFO)unexpected affiliation role with parliamentary group organization</xsl:when>
