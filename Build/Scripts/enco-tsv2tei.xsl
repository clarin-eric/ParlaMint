...
<!-- Insert encoder meta-data on political parties from TSV file into <listOrg>.
     Input TSVs must have appropriate header row
     Existing encoder meta-data on such orientation in <listOrg> is removed.

     Fake example:
     TSV input:

     country	pm_id	lr	encoder		comment
     BE		CD&V	CR	GrietDepoorter	Orientation determined by encoder.
    ...
    <org xml:id="parliamentaryGroup.BZÖ" role="parliamentaryGroup" xml:lang="de">
      ...
      <state type="politicalOrientation">
         <state type="encoder" source="#GrietDepoorter" ana="#political.CR">
           <note xml:lang="en">Orientation determined by encoder</note>
        </state>
      </state>
    </org>
-->
	...
	<org>
	  ...
	  <xsl:choose>
	    <xsl:when test="not(normalize-space($lr))">
              <xsl:message select="concat('WARN: For ', $country, ' ', $pm_id, ' no orientation in encoder TSV')"/>
	    </xsl:when>
	    <xsl:otherwise>
	      <state type="encoder" source="#{$encoder}" ana="{concat($Orientation-prefix, $lr)}">
